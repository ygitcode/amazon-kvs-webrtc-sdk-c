...
a=ice-options:trickle
    ...
    auto sdpOfferAudio1 = R"(m=audio 9 UDP/TLS/RTP/SAVPF 109 9 0 8 101
c=IN IP4 0.0.0.0
a=recvonly
a=extmap:1 urn:ietf:params:rtp-hdrext:ssrc-audio-level
...
a=ssrc:1326673407 cname:{080eadab-2d5f-4ba3-abc2-0ccc305a028d})";
    ...
    auto sdpOfferVideo1 = R"(m=video 9 UDP/TLS/RTP/SAVPF 97 98 99 100 101 122 102 121 127 120 125 107 108 109 124 119 123
c=IN IP4 0.0.0.0
a=rtcp:9 IN IP4 0.0.0.0
a=ice-ufrag:XvdX
...
a=rtpmap:123 ulpfec/90000)";
    ...
    auto sdpOfferVideo2 = R"(m=video 9 UDP/TLS/RTP/SAVPF 124 121 125 126 127 97 98
c=IN IP4 0.0.0.0
a=recvonly
a=extmap:3 urn:ietf:params:rtp-hdrext:sdes:mid
...
a=ssrc:1644235696 cname:{36a6a74c-73a4-594b-9bb0-023b4d357280})";
    ...
    offer += sdpaudio_sendrecv_mid0;
    offer += "\n";
    offer += sdpOfferAudio1;
    offer += "\n";
    offer += sdpOfferVideo1;
    offer += "\n";
    offer += sdpvideo;
    offer += "\n";
    ...
    assertLFAndCRLF((PCHAR) offer.c_str(), offer.size(), [](PCHAR sdp) {
        ...
        // check m-line and codec value
        std::string::size_type posMline1 = answer.find("m=audio 9 UDP/TLS/RTP/SAVPF 111");
        EXPECT_NE(std::string::npos, posMline1);
        posMline1++;
        std::string::size_type posMline2 = answer.find("m=audio 9 UDP/TLS/RTP/SAVPF 109", posMline1);
        EXPECT_NE(std::string::npos, posMline2);
        posMline2++;
        std::string::size_type posMline3 = answer.find("m=video 9 UDP/TLS/RTP/SAVPF 97", posMline2);
        EXPECT_NE(std::string::npos, posMline3);
        posMline3++;
        std::string::size_type posMline4 = answer.find("m=video 9 UDP/TLS/RTP/SAVPF 96", posMline3);
        EXPECT_NE(std::string::npos, posMline4);
        posMline4++;
        std::string::size_type posMline5 = answer.find("m=video 9 UDP/TLS/RTP/SAVPF 124", posMline4);
        EXPECT_NE(std::string::npos, posMline5);

        // check order of the m-lines
        EXPECT_EQ(TRUE, posMline1 < posMline2);
        EXPECT_EQ(TRUE, posMline2 < posMline3);
        EXPECT_EQ(TRUE, posMline3 < posMline4);
        EXPECT_EQ(TRUE, posMline4 < posMline5);

        // check directions
        EXPECT_NE(std::string::npos, answer.substr(posMline1, posMline2 - posMline1).find("a=inactive"));
        EXPECT_NE(std::string::npos, answer.substr(posMline2, posMline3 - posMline2).find("a=inactive"));
        EXPECT_NE(std::string::npos, answer.substr(posMline3, posMline4 - posMline3).find("a=inactive"));
        EXPECT_NE(std::string::npos, answer.substr(posMline4, posMline5 - posMline4).find("a=sendonly"));
        EXPECT_NE(std::string::npos, answer.substr(posMline5).find("a=inactive"));

        // check rtpmap values
        EXPECT_NE(std::string::npos, answer.substr(posMline1, posMline2 - posMline1).find("a=rtpmap:111 opus/48000/2"));
        EXPECT_NE(std::string::npos, answer.substr(posMline2, posMline3 - posMline2).find("a=rtpmap:109 opus/48000/2"));
        EXPECT_NE(std::string::npos, answer.substr(posMline3, posMline4 - posMline3).find("a=rtpmap:97 rtx/90000"));
        EXPECT_NE(std::string::npos, answer.substr(posMline5).find("a=rtpmap:124 rtx/90000"));
